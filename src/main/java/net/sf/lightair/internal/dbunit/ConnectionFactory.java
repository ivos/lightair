...
public class ConnectionFactory implements PropertyKeys {
	 ...
	 */
	public IDatabaseConnection createConnection(String profile, String schemaName)
		...
		IDatabaseConnection connection = factory.createDatabaseConnection(profile, schemaName);
		...
		stopWatch.stop();
		log.debug("Created database connection for schema {} in {} ms.", schemaName, stopWatch.getTime());
		return connection;
	}
	...
	private void setDatabaseDialect(String profile, DatabaseConfig config) {
		...
		String dialect = propertiesProvider.getProperty(profile, DATABASE_DIALECT);
		String className = DATATYPE_FACTORIES.get(dialect);
		Object value = convertPropertyValue(className);
		log.debug("Setting database dialect to {}, DbUnit datatype factory will be {}.", dialect, className);
		config.setProperty(dbUnitName, value);
	}

	private void setStatementFactory(DatabaseConfig config) {
		config.setProperty(DatabaseConfig.PROPERTY_STATEMENT_FACTORY, Factory.getInstance().getStatementFactory());
	}
...

	{
		...
		DATATYPE_FACTORIES.put("informix", "net.sf.lightair.internal.dbunit.dataset.datatype.InformixDataTypeFactory");
	}

	private void setFeaturesAndProperties(String profile, DatabaseConfig config) {
		Set<String> featureNames = propertiesProvider.getDbUnitFeatureNames(profile);
		for (String featureName : featureNames) {
			String dbUnitName = "http://www.dbunit.org/features/" + featureName.substring(16);
			Boolean value = Boolean.valueOf(propertiesProvider.getProperty(profile, featureName));
			log.debug("Setting DbUnit feature {} to {}.", dbUnitName, value);
			config.setProperty(dbUnitName, value);
		}

		Set<String> propertyNames = propertiesProvider.getDbUnitPropertyNames(profile);
		for (String propertyName : propertyNames) {
			String dbUnitName = "http://www.dbunit.org/properties/" + propertyName.substring(18);
			String string = propertiesProvider.getProperty(profile, propertyName);
